	...
	public static void main(String[] args) {
		...
		if (max_amperes != Boite.AMPERAGE_MIN - 1) {
			...
			do {
				...
				// Boucle qui attend que l'utilisateur appuie sur un des
				// boutons.
				...
				/***/// Selon la version de Java, il est possible d'utiliser
						// switch-case.
				if (option.equals(
					Constantes.OPTIONS_MENU[Constantes.AJOUTER_DISJONCTEUR])){

					UtilitaireGestionMenu.ajouterDisjoncteur(boite);
				}

				else if (option.equals(
						Constantes.OPTIONS_MENU[Constantes.AJOUTER_APPAREIL])){
					UtilitaireGestionMenu.ajouterDemande(boite);
				}

				else if (option.equals(
						Constantes.OPTIONS_MENU[Constantes.RECUPERER])) {

					// On essaie dans une autre boite car si null, on veut
					// garder la
					...
					if (boiteTmp != null) {
						...
						Constantes.OPTIONS_MENU[Constantes.SAUVEGARDER])) {
